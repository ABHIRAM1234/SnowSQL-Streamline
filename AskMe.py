...
def get_openai_response(question):
    ...
    # st.write(response['choices'][-1]['message']['content'])
    response1 = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": f"{response['choices'][-1]['message']['content']}"},
            {"role": "user", "content": f"Run the SQL query on the columns {df} to fetch the answer from table. Display both the generated query and answer from the table as SQL:\n generated query and Answer:\n result from the table"}
        ],
        temperature=0,
        max_tokens=256,
        top_p=1,
    ...
    # st.write(response1['choices'][-1]['message']['content'])
    return response1['choices'][-1]['message']['content']
